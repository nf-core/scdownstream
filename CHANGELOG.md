...
- Fold the `main_pipeline_build` pipeline test into the `default` test: `-profile test` now also runs Seurat integration and uses the `Adult_COVID19_PBMC` CellTypist model to match the PBMC test data.
- Size CPU and memory per process for `-profile test` pipeline runs (`conf/test_resources.config`), based on measured peak usage, so that several tasks share a runner and the pipeline nf-tests run faster.
