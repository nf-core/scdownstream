...
H5AD files in the samplesheet may use either string encoding, including the `nullable-string-array` encoding written by anndata 0.13 with pandas 3. The pipeline itself always writes the legacy string encoding (`ANNDATA_ALLOW_WRITE_NULLABLE_STRINGS=0` in `nextflow.config`), because the R-based modules read H5AD with anndataR, which does not support nullable strings yet.
