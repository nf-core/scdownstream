...
for celltype, celltype_data in adata.obs.groupby("celltype", observed=True):
    ...
    for treatment in treatments:
        ...
        write_volcano(
            ...
            f"{stem}_volcano.png",
            f"{stem}_volcano",
            f"PyDESeq2 volcano ({celltype}, {treatment} vs {reference_condition})",
            f"PyDESeq2 volcano for <code>{celltype}</code>, contrast <code>{treatment}</code> vs <code>{reference_condition}</code>.",
        )
