...
volcano_parent = mqc_parent("pydeseq2", "PyDESeq2")
written = []
for celltype, celltype_data in adata.obs.groupby("celltype", observed=True):
    ...
    for treatment in treatments:
        ...
        write_volcano(
            ...
            f"{stem}_volcano.png",
            f"{stem}_volcano",
            f"PyDESeq2 volcano: {treatment} vs {reference_condition} (within celltype={celltype})",
            (
                f"PyDESeq2 volcano for contrast <code>{treatment}</code> versus <code>{reference_condition}</code> "
