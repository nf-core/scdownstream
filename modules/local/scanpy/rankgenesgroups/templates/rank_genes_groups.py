...
if len(valid_groups) >= 2:
    ...
    stratum = f"{filter_col}={filter_val}" if filter_col and filter_val else ""
    groups = full_df["group"].astype(str)
    write_standard_de_parquet(
        full_df,
        f"{prefix}_results.parquet",
        gene_col="names",
        log2fc_col="logfoldchanges",
        pvalue_col="pvals" if "pvals" in full_df.columns else None,
        padj_col="pvals_adj" if "pvals_adj" in full_df.columns else None,
        group=groups,
        contrast=groups + " vs rest",
        stratum=stratum,
    )
